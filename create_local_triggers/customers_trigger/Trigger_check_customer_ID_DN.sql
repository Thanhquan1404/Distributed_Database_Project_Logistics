...
BEGIN
    ...
    -- Kiểm tra tiền tố ID Khách hàng
    IF :NEW.Customer_ID NOT LIKE 'CUSTDN%' THEN
        RAISE_APPLICATION_ERROR(-20023, 'Lỗi bảo mật Site DN: Customer_ID bắt buộc phải bắt đầu bằng "CUSTDN"!');
    END IF;
    
    -- Kiểm tra tính hợp lệ của phân vùng dữ liệu dựa trên Thành phố
    IF v_region != 'DN' OR v_region IS NULL THEN
        RAISE_APPLICATION_ERROR(-20024, 'Lỗi bảo mật Site DN: Thành phố ' || :NEW.City || ' không thuộc phạm vi quản lý của chi nhánh Đà Nẵng!');
    END IF;
END;
/
