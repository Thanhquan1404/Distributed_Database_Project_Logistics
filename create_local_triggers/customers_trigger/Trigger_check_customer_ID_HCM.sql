...
BEGIN
    ...
    IF :NEW.Customer_ID NOT LIKE 'CUSTHCM%' THEN
        RAISE_APPLICATION_ERROR(-20023, 'Lỗi bảo mật Site DN: Customer_ID bắt buộc phải bắt đầu bằng "CUSTHCM"!');
    END IF;
    
    IF v_region != 'HCM' OR v_region IS NULL THEN
        RAISE_APPLICATION_ERROR(-20024, 'Lỗi bảo mật Site DN: Thành phố ' || :NEW.City || ' không thuộc phạm vi quản lý của chi nhánh Hồ Chí Minh!');
    END IF;
END;
/
