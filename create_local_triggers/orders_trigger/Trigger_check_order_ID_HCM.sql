...
BEGIN
    ...
    IF :NEW.Order_ID NOT LIKE 'ORDHCM%' THEN
        RAISE_APPLICATION_ERROR(-20025, 'Lỗi bảo mật Site DN: Order_ID bắt buộc phải bắt đầu bằng "ORDHCM"!');
    END IF;
    
    IF v_hub_region != 'HCM' OR v_hub_region IS NULL THEN
        RAISE_APPLICATION_ERROR(-20026, 'Lỗi bảo mật Site DN: Kho gốc (Root_Hub) của đơn hàng này phải thuộc phân vùng HỒ CHÍ MINH!');
    END IF;
END;
/