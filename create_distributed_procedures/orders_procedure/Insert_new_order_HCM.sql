...
BEGIN
    ...
    IF v_cust_exists = 0 THEN
        SELECT COUNT(*) INTO v_cust_exists FROM Customers@ThanhQuan_Oracle_DistributedDatabase WHERE Customer_ID = v_Customer_ID;
    END IF;
    IF v_cust_exists = 0 THEN
    ...
    IF v_root_region = 'DN' THEN
        INSERT INTO Orders_Ops@ThanhQuan_Oracle_DistributedDatabase (Order_ID, Customer_ID, Product_ID, Root_Hub, Dest_Hub, Mode_Of_Shipment, Status, Created_By)
        VALUES (v_Order_ID, v_Customer_ID, v_Product_ID, v_Root_Hub, v_Dest_Hub, v_Mode_Of_Shipment, v_Status, 'HCM');
    ...
    INSERT INTO Orders_Payment (Order_ID, Amount, Pay_Status, Created_By)
    VALUES (v_Order_ID, v_Amount, v_Pay_Status, 'HCM');
