...
BEGIN
    OPEN p_cursor FOR
        SELECT
            ...
            o.Root_Hub,
            o.Dest_Hub,
            get_region_of_hub(o.Root_Hub) AS Origin_Region,  -- Vùng xuất phát
            get_region_of_hub(o.Dest_Hub) AS Dest_Region,    -- Vùng đích
            o.Mode_Of_Shipment,
            o.Status,
            o.Created_At,
            o.Created_By,
            o.Updated_By
        FROM (
            SELECT * FROM Orders_Ops                                      -- DN
            UNION ALL
            SELECT * FROM Orders_Ops@MinhThuy_Oracle_DistributedDatabase  -- HN
            UNION ALL
            SELECT * FROM Orders_Ops@KimNgan_Oracle_DistributedDatabase    -- HCM
        ) o
