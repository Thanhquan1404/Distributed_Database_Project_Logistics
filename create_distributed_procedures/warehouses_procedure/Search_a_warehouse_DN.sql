...
BEGIN
    OPEN p_cursor FOR
        ...
        FROM (
            ...
            UNION ALL
            SELECT * FROM Warehouses@KimNgan_Oracle_DistributedDatabase
        ) w
...
BEGIN
    Search_Warehouses(
        ...
        v_Hub_Name    => NULL, -- Thay đổi để search theo HUB_NAME, nếu set NULL sẽ search toàn bộ Hub_ID
        v_Region_Code => 'DN'  -- Thay đổi để search theo REGION_CODE (HN, HCM, DN), nếu set NULL sẽ search toàn bộ Hub_ID
    );
