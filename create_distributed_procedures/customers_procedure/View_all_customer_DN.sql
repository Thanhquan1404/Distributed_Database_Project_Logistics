...
BEGIN
    OPEN p_cursor FOR
        SELECT
            ...
            c.Customer_Name,
            c.City,
            get_region_from_city(c.City) AS Region,  -- Hiển thị thêm vùng
            c.Customer_Rating,
            c.Created_At,
            c.Created_By,
            c.Updated_By
        FROM (
            SELECT * FROM Customers                                      -- DN
            UNION ALL
            SELECT * FROM Customers@MinhThuy_Oracle_DistributedDatabase  -- HN
            UNION ALL
            SELECT * FROM Customers@KimNgan_Oracle_DistributedDatabase    -- HCM
        ) c
