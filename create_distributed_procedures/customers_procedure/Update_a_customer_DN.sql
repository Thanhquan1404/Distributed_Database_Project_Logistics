...
CREATE OR REPLACE PROCEDURE Update_Customer(
    ...
    v_New_City         NVARCHAR2 DEFAULT NULL,  -- Nếu đổi City phải cùng vùng
...
BEGIN
    ...
    ELSE
        ...
        ELSE
            ...
            IF v_count > 0 THEN
                v_target_link := 'HCM';
                SELECT City INTO v_old_city FROM Customers@KimNgan_Oracle_DistributedDatabase WHERE Customer_ID = v_Customer_ID;
            ELSE
    ...
    ELSIF v_target_link = 'HN' THEN
        ...
        WHERE Customer_ID = v_Customer_ID;
    ELSE
        UPDATE Customers@KimNgan_Oracle_DistributedDatabase
        SET Customer_Name = NVL(v_Customer_Name, Customer_Name),
            City = NVL(v_New_City, City),
            Customer_Rating = NVL(v_Customer_Rating, Customer_Rating),
            Updated_By='DN'
